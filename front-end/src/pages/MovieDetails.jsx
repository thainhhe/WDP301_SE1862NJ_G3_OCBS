...
const MovieDetails = () => {
  ...
  const fetchMovieDetails = async () => {
    try {
      ...
      const [movieData, showtimesData] = await Promise.all([
        ...
        showtimeService?.getShowtimes({ movieId: id, limit: 10 }),
      ])
  ...
  const getStatusBadge = (status) => {
    ...
    return <span className={`${config.color} text-white text-sm font-bold px-3 py-1 rounded-full`}>{config.text}</span>
  }
            ...
            <div className="container mx-auto px-4">
              <div className="max-w-2xl">
                ...
                <span className="flex items-center">
                  ...
                  {movie.hotness > 0 && <span className="flex items-center">🔥 {movie.hotness}/10</span>}
                </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-6 sticky top-8">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">{formatDuration(movie.duration)}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">{new Date(movie.releaseDate).toLocaleDateString()}</p>
                  </div>
                  ...
                  {movie.hotness > 0 && (
                      <div>
                        ...
                        <div className="flex items-center mt-1">
                          ...
                          <span className="text-sm font-bold text-red-600">{movie.hotness}/10</span>
                        </div>
                      </div>
                  )}
                </div>
                ...
                {movie.status === "now-showing" && showtimes.length > 0 && (
                    <div className="mt-8">
                      <h4 className="text-lg font-bold text-gray-900 mb-4">Available Showtimes</h4>
                ...
                {movie.status === "now-showing" && (
                    <div className="mt-6">
                      <Link
                          to={`/showtimes?movieId=${movie._id}`}
                          className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-4 rounded-lg transition duration-300 inline-block text-center"
                      >
                        Book Tickets
                      </Link>
                    </div>
